...
async fn main()  {
    ...
    for row_file in &file_items {
        ...
        //println!("con.len={}", row_file.content.len());
        let request = EmbeddingRequest {
            model: model_embed_name.to_string(),
            prompt: row_file.content.to_string(),
        };
        ...
        if res.status().is_success() {
            ...
            if response_body.embedding.len() > 0 {
                ...
                let bytes_u8 = f32_vec_to_u8_vec(&response_body.embedding);
                ...
                .bind(new_id)
                .bind(&sessid)
                .bind(&row_file.name.clone())
                .bind(&row_file.content.clone())
                .bind(&bytes_u8)
                .execute(&pool)
                .await.unwrap();
            } else {
